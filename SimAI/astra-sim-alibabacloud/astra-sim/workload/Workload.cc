...
Workload::Workload(
  ...
  // Initialize PP rank and neighbors
  {
    int TP_size = model_parallel_npu_group == 0 ? generator->total_nodes : model_parallel_npu_group;
    pp_size = pipeline_model_parallelism > 0 ? pipeline_model_parallelism : 1;
    pp_rank = (generator->id / TP_size) % pp_size;
    pp_prev = (pp_rank > 0) ? (generator->id - TP_size) : -1;
    pp_next = (pp_rank < pp_size - 1) ? (generator->id + TP_size) : -1;
    pp_recv_pending = false;
    pp_comm_issued = false;
    if (generator->id == 0) {
      std::cout << "PP config: pp_size=" << pp_size
                << " pp_commsize=" << pp_commsize << std::endl;
    }
    if (pp_size > 1) {
      std::cout << "GPU " << generator->id << ": pp_rank=" << pp_rank
                << " pp_prev=" << pp_prev << " pp_next=" << pp_next << std::endl;
    }
  }
...
void Workload::pp_recv_done_callback(void* arg) {
  Workload* wl = static_cast<Workload*>(arg);
  wl->pp_recv_pending = false;
  wl->generator->register_event(wl, EventType::General, NULL, 1);
}
...
void Workload::issue_pp_communication(bool is_backward) {
  ...
  uint64_t comm_size = pp_commsize;
  int tag = 2000000 + pass_counter * 2 + (is_backward ? 1 : 0);
  sim_request req;
  memset(&req, 0, sizeof(req));
  req.flowTag.tag_id = tag;
  req.flowTag.current_flow_id = tag;
  req.flowTag.child_flow_id = -1;

  if (!is_backward) {
    // Forward: send activation to next stage, recv activation from prev stage
    if (pp_next >= 0) {
      req.srcRank = generator->id;
      req.dstRank = pp_next;
      req.tag = tag;
      req.reqCount = comm_size;
      generator->front_end_sim_send(
          0, Sys::dummy_data, comm_size, UINT8,
          pp_next, tag, &req, nullptr, nullptr);
    }
    if (pp_prev >= 0) {
      req.srcRank = pp_prev;
      req.dstRank = generator->id;
      req.tag = tag;
      req.reqCount = comm_size;
      pp_recv_pending = true;
      generator->front_end_sim_recv(
          0, Sys::dummy_data, comm_size, UINT8,
          pp_prev, tag, &req,
          &Workload::pp_recv_done_callback, this);
    }
  } else {
    // Backward: send gradient to prev stage, recv gradient from next stage
    if (pp_prev >= 0) {
      req.srcRank = generator->id;
      req.dstRank = pp_prev;
      req.tag = tag;
      ...
      generator->front_end_sim_send(
          0, Sys::dummy_data, comm_size, UINT8,
          pp_prev, tag, &req, nullptr, nullptr);
    }
    if (pp_next >= 0) {
      req.srcRank = pp_next;
      req.dstRank = generator->id;
...
void Workload::iterate_hybrid_parallel_Transformer() {
  ...
  if (current_state == LoopState::Forward_Pass) {
    ...
    if (index >= SIZE) {
      index--;
      if (pp_size > 1) {
        current_state = LoopState::PP_Forward_Send_Recv;
        generator->register_event(this, EventType::General, NULL, 1);
        return;
      }
      current_state = LoopState::Input_Gradient;
    ...
    generator->register_event(this, EventType::General, NULL, 1);
    return;
  } else if (current_state == LoopState::PP_Forward_Send_Recv) {
    if (!pp_comm_issued) {
      pp_comm_issued = true;
      issue_pp_communication(false);
      if (pp_recv_pending) return;
    }
    if (pp_recv_pending) return;
    pp_comm_issued = false;
    current_state = LoopState::Input_Gradient;
    ...
    generator->register_event(this, EventType::General, NULL, 1);
    return;
  } else if (current_state == LoopState::PP_Backward_Send_Recv) {
    if (!pp_comm_issued) {
      pp_comm_issued = true;
      issue_pp_communication(true);
      if (pp_recv_pending) return;
    }
    if (pp_recv_pending) return;
    pp_comm_issued = false;
...
bool Workload::initialize_workload(std::string name) {
 ...
 std::string firstline;
  ...
  for (int i = 0; i < lines; i++) {
    ...
    // Override involved dimensions for EP group types:
    // When expert_parallel_npu_group > 1, EP collectives must activate
    // dimension 0 so traffic actually flows through the network.
    // Without this, break_dimension(model_parallel_npu_group=1) returns -1,
    // making all fwd/ig dimensions false, which causes "all dims disabled".
    if (expert_parallel_npu_group > 1) {
      if (fp_group_type == MockNccl::GroupType::EP &&
          fp_type != ComType::None &&
          selected_involved_dimensions["fwd"].size() > 0) {
        selected_involved_dimensions["fwd"][0] = true;
      }
      if (ig_group_type == MockNccl::GroupType::EP &&
          ig_type != ComType::None &&
          selected_involved_dimensions["ig"].size() > 0) {
        selected_involved_dimensions["ig"][0] = true;
      }
      if (wg_group_type == MockNccl::GroupType::DP_EP &&
          wg_type != ComType::None &&
          selected_involved_dimensions["wg"].size() > 0) {
        selected_involved_dimensions["wg"][0] = true;
      }
    }
    Layer* l = new Layer(
        id,
