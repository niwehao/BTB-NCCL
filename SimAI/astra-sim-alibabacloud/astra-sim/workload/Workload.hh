...
class Workload : Callable {
 public:
  enum class LoopState {
    ...
    Wait_For_Sim_Finish,
    Forward_In_BackPass,
    PP_Forward_Send_Recv,
    PP_Backward_Send_Recv
  };
  ~Workload();
  Layer** layers;
  int SIZE;
  ...
  int pp_rank;
  int pp_prev;           // global rank of previous PP stage, -1 if first
  int pp_next;           // global rank of next PP stage, -1 if last
  int pp_size;
  bool pp_recv_pending;
  bool pp_comm_issued;
  ...
  void issue_pp_communication(bool is_backward);
  static void pp_recv_done_callback(void* arg);
