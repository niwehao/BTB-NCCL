...
mixtral_default_path="$SCRIPT_DIR/inference_configs/mixtral_8x7b.json"
...
Options:
  -m, --model-size <SIZE>
      Model size to use.
      Possible values: {deepseek-671B, qwen3-235B, qwen3-next-80B, mixtral-8x7B}.
      (Default: $model_size)
...
case $model_size in
  ...
  mixtral-8x7B) # 新增这个分支
    ...
    config_file_path=${config_file_path:-$mixtral_default_path}
    # 自动根据模型特性开启 MoE
    moe_enable=true
    ;;
  qwen3-235B)
