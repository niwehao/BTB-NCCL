...
void DemandRecorder::get_unsatisfied_demand(Matrix2D<double> & tm, int enable_tcpinfo){
  if(enable_tcpinfo){
    tm.fill_zeros();
    // simtime_picosec now = eventlist().now();
    //list<TcpSrc*, tcp_info*>::iterator i = rtx_scanner->_tcppairs.begin();
    list<pair<TcpSrc*, tcp_info*>>::iterator i =  rtx_scanner->_tcppairs.begin();
    while (i != rtx_scanner->_tcppairs.end())
    {
      TcpSrc* tcp_src = i->first;        // 访问 TcpSrc* 指针
      tcp_info* tcp_info = i->second;    // 访问 tcp_info* 指针
      if (tcp_src->_finished)
      {
        tcp_src->eventlist().cancelPendingSource(*tcp_src);
        // delete *i;
        i = rtx_scanner->_tcppairs.erase(i);
      }
      else
      {
        tm.add_elem_by(tcp_src->_flow_src, tcp_src->_flow_dst, tcp_src->_flow_size - tcp_src->_last_acked);
...
{
  std::cerr <<"DynFlatScheduler::doNextEvent()"<<std::endl;
...
{
  update_all_queue_bandwidth();//adjust bandwidth of each link
  update_all_route();// so... why we need to update route....
  status = DynNetworkStatus::DYN_NET_RECONF;
  eventlist.sourceIsPendingRel(*this, reconf_delay);
}
...
void DynFlatScheduler::update_all_route()
{
  ...
  while (i != demandrecorder->rtx_scanner->_tcps.end())
  {
    ...
    // else
    {
      int src = tcpsrc->_flow_src;
      int dst = tcpsrc->_flow_dst;
      int choice = 0;
      Route* routeout, *routein;
      vector<const Route*>* srcpaths = topo->get_paths(src, dst);
      choice = rand()%srcpaths->size(); // comment this out if we want to use the first path
      routeout = new Route(*(srcpaths->at(choice)));
      routeout->push_back(tcpsrc->_sink);

      choice = 0;
      vector<const Route*>* dstpaths = topo->get_paths(dst, src);
