...
{
  _ratio_high = 640; // 640:1500 is a 10:1 ratio of (64-byte) headers to (1500-byte) packets
  _ratio_low = 1500;
  _crt = 0;
  _num_headers = 0;
  _num_packets = 0;
...
CompositeQueue::doNextEvent() {
  std::cerr <<"CompositeQueue::doNextEvent()"<<std::endl;
  completeService();
}
...
CompositeQueue::receivePacket(Packet& pkt)
{
    ...
    if (!pkt.header_only()) {
		...
		if (_queuesize_low+pkt.size() <= _maxsize  || drand()<0.5) {
	    	...
	    	if (chk) {
	    		// the new packet fit, prioritize the booted packet header
