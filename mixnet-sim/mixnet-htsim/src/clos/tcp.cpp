...
void TcpSrc::set_app_limit(int pktps)
{
	...
	_ssthresh = 0xffffffff;
	_app_limited = pktps;
	if (!TcpSrc::tcp_flow_paused && !tcp_flow_paused_in_region)
		send_packets();
	else
	{
...
{
	// std::cerr <<"TcpSrc::doNextEvent()"<<std::endl;
	if (_rtx_timeout_pending)
	{
		_rtx_timeout_pending = false;
...
void TcpSrc::pause_flow_in_region()
{
	tcp_flow_paused_in_region = true;
}
...
void TcpSrc::resume_flow_in_region()
{
	tcp_flow_paused_in_region = false;
}

void TcpSrc::resume_flow()
{
	// tcp_flow_paused & tcp_flow_paused_in_region could share this logic to resume flow
...
TcpRtxTimerScanner::TcpRtxTimerScanner(simtime_picosec scanPeriod, EventList &eventlist, int enable_tcppairs)
		: EventSource(eventlist, "RtxScanner"), _scanPeriod(scanPeriod)
{
	eventlist.sourceIsPendingRel(*this, _scanPeriod);
	_enable_tcppairs=enable_tcppairs;
}

TcpRtxTimerScanner::TcpRtxTimerScanner(simtime_picosec scanPeriod, EventList &eventlist, int enable_tcppairs, int micro_batch_num, int layer_num, int n_cols,int n_rows)
		: EventSource(eventlist, "RtxScanner"), _scanPeriod(scanPeriod), _enable_tcppairs(enable_tcppairs)
{
	traffic_matrix.reserve(micro_batch_num);  // 预留空间以避免重复分配
    for (int i = 0; i < micro_batch_num; ++i) {
        std::vector<Matrix2D<double>> layer_vector;
        layer_vector.reserve(layer_num);  // 预留空间以避免重复分配
        for (int j = 0; j < layer_num; ++j) {
            layer_vector.emplace_back(n_cols, n_rows);  // 直接在 vector 中构造 Matrix2D 对象
        }
        traffic_matrix.emplace_back(std::move(layer_vector));  // 将构造好的 vector 移动到 traffic_matrix 中
    }
	eventlist.sourceIsPendingRel(*this, _scanPeriod);
}
...
void TcpRtxTimerScanner::registerTcp(TcpSrc &tcpsrc,tcp_info &tcpinfo){
	TcpPair tcp_pair(&tcpsrc, &tcpinfo);
	_tcppairs.push_back(tcp_pair);
}
...
void TcpRtxTimerScanner::doNextEvent()
{
	/*
	TODO:
		1. check the traffic matrix only contain half of the matrix
	*/
	if (_enable_tcppairs) {
		simtime_picosec now = eventlist().now();
		TcpPairList::iterator i = _tcppairs.begin();
		while (i != _tcppairs.end())
		{
			TcpSrc* tcp_src = i->first;        // 访问 TcpSrc* 指针
    		tcp_info* tcp_info = i->second;    // 访问 tcp_info* 指针

			if ((tcp_src)->_finished)
			{
				// only collect the traffic of the first groupby forward all2all
				// seems like we only need keep microbatch * traffic matrix
				if(tcp_info->tcp_type==tcp_info::all2all_groupby_forward) {
					traffic_matrix[tcp_info->micro_batch_id][tcp_info->layer_id].add_elem_by(tcp_src->_flow_src, tcp_src->_flow_dst, tcp_src->_flow_size);
				}
				eventlist().cancelPendingSource(*tcp_src);
				// delete *i;
				i = _tcppairs.erase(i);
			}
			else
			{
				tcp_src->rtx_timer_hook(now, _scanPeriod);
				i++;
			}
		}
		eventlist().sourceIsPendingRel(*this, _scanPeriod);
