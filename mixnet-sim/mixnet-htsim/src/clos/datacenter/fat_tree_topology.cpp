...
FatTreeTopology::FatTreeTopology(int no_of_nodes, mem_b queuesize, Logfile *lg,
                                 EventList *ev, FirstFit *fit, queue_type q, uint32_t speed)
{
  _queuesize = queuesize;
  logfile = lg;
  eventlist = ev;
...
void FatTreeTopology::set_params(int no_of_nodes)
{
  ...
  cout << "Building 1:1 Fat Tree with " << _no_of_nodes << " nodes (k=" << K << ")" << endl;
  if (link_speed != 0) {
    cout << "Link speed: " << link_speed << " Mbps" << endl;
  } else {
    cout << "Link speed: " << SPEED << " Mbps" << endl;
  }
  
  NK = (K * K / 2);       // number of switches in a tier (if NOT the top tier)
  NC = (K * K / 4);       // number of switches in the top tier (core)
  NSRV = (K * K * K / 4); // number of servers
  //cout << "Fat Tree parameters:" << endl;
  //cout << "   Number of nodes " << _no_of_nodes << endl;
  //cout << "   K = " << K << endl;
...
Queue *FatTreeTopology::alloc_src_queue(QueueLogger *queueLogger)
{
  if(link_speed == 0) {
    ...
    return new PriorityQueue(speedFromMbps((uint64_t)link_speed), memFromPkt(FEEDER_BUFFER), *eventlist, queueLogger);
  }
}
...
Queue *FatTreeTopology::alloc_queue(QueueLogger *queueLogger, mem_b queuesize)
{
  if(link_speed == 0) {
    ...
    return alloc_queue(queueLogger, link_speed, queuesize);
  }
}
