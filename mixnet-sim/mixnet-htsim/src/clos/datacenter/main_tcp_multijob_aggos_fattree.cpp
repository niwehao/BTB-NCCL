...
int main(int argc, char **argv)
{
  ...
  string flowfiles = "../../../test/taskgraph.fbuf";       // so we can read the flows from a specified file
  ...
  AggOverSubscribedFatTree* top = 
    new AggOverSubscribedFatTree(k, nup, queuesize, nullptr /* &logfile */, &eventlist, ff, ECN);
  ...
  for (int i = 0; i < flowfile_arr.size(); i++) {
    ...
    std::cerr << endl;
    FFApplication * app = new FFApplication(top, ssthresh, logdir, &fct_util_out, nodes, tcpRtxScanner, eventlist);
