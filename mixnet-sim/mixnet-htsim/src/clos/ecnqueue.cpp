...
void ECNQueue::receivePacket(Packet &pkt)
{
...
void ECNQueue::set_regional_topo_manager(RegionalTopoManager * regional_topo_manager_)
{
...
void ECNQueue::completeService()
{
	/* dequeue the packet */
	if (_enqueued.empty()) {
		// Queue was drained during reconfiguration — nothing to do
		return;
	}
	...
	// Note: Regional reconfig is now immediate (no drain wait), so no need to
	// check regional_topo_manager->do_reconf() on every packet dequeue.
