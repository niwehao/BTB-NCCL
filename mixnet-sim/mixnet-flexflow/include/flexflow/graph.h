...
public:
  ...
  void create_resource_pool(int num_nodes,
                            int num_gpus_per_node);
  
  // std::shared_ptr <spdlog::basic_logger_mt> graph_logger;
  // void init_logger();
  //***********************************************************************//
  ...
  std::unique_ptr<Graph> generate_pp_graph(Graph const &g, 
                          int train_pp, 
                          int layer_per_stage,
                          std::unordered_map<Node, MachineView> const &strategy,
