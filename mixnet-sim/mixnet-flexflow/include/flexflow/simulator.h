...
public:
  ...
  bool store;
  std::string name;
  char* task_info;//record the info of group
  std::vector<int> from_node_ids;
  std::vector<int> to_node_ids;
...
public:
  ...
  void reset(size_t  dp,size_t tp,size_t pp,size_t ep);
  void reset();
  ...
  SimTask *new_p2p_task(int src_id, int dst_id, size_t message_size);
  SimTask *new_p2p_task(int src_id,
                        int dst_id,
                        size_t message_size,
                        int micro_batch,
                        int layer_id,
                        ...
                        int TO_layer_id,
                        int type);
  SimTask *new_p2p_task(std::vector<int> const &src_ids,
  ...
  SimTask *new_p2p_task(std::vector<int> const &src_ids,
                        ...
                        int TO_layer_id,
                        int type);
  SimTask *new_sub_allreduce_task(int node_id); //syj: suballreduce task, to get the group node
  SimTask *new_dp_allreduce_task(
                              std::vector<int> const &node_ids,
                              size_t message_size);//syj: no need to add next task
  SimTask *new_tp_allreduce_task(
                                std::vector<int> const &node_ids,
                                size_t message_size);//have next task to add
  SimTask *new_allreduce_task(Op *op,
                                std::vector<int> const &node_ids,
                                size_t message_size);//have next task to add
  SimTask *new_allreduce_task(
                              std::vector<int> const &node_ids,
                              size_t message_size);//syj: no need to add next task
  SimTask *new_alltoall_task(
  ...
  //with microbatch info and layer id
  SimTask *new_backward_task(Op const *op, int idx, int micro_batch, int layer_id);
  SimTask *new_forward_task(Op const *op, int idx, int micro_batch, int layer_id);
  SimTask *new_allreduce_task( std::vector<int> const &node_ids,
                              size_t message_size,
                              int micro_batch, 
                              int layer_id,
                              int type // tp fw; tp bw; bp fw;bp bw
                              );
  SimTask *new_alltoall_task(
                            std::vector<int> const &from_node_ids,
                            std::vector<int> const &to_node_ids,
                            ...
                            );//have next task to add
public:
  size_t global_task_id, max_num_tasks;
  SimTask **tasks;
  size_t dp_degree,tp_degree,pp_degree,ep_degree;
...
public:
  ...
  float default_estimate_sync_cost(const ParallelTensor tensor,
                                   MachineView const &view,
                                   int num_replicate_dims);
  virtual float simulate_runtime(FFModel const *model,
  ...
  static void simulation_task(Legion::Task const *task,
  ...
  static void
      strategy_search_task(Legion::Task const *task,
                           ...
                           Legion::Context ctx,
                           Legion::Runtime *runtime);
  void fortest_helloworld();//Op const *op
...
public:
  ...
  void add_task_dependencies_with_xfer(SimTask *src_task,
                                       SimTask *dst_task,
                                       size_t message_size);
  bool serialize_logical_taskgraph(FFModel const *model, std::string const &export_file_name);
  static void
      simulation_task(Legion::Task const *task,
                      std::vector<Legion::PhysicalRegion> const &regions,
                      Legion::Context ctx,
                      Legion::Runtime *runtime);
  virtual void get_taskgraph_flatbuf(FFModel const *model, flatbuffers::FlatBufferBuilder &builder);
...
class TaskgraphSimulator : public LogicalTaskgraphBasedSimulator {
public:
  TaskgraphSimulator(FFModel const *model,
                     FFHandler handler,
                     Legion::Memory memory,
                     MachineModel *machine);
  virtual float simulate_runtime(FFModel const *model,
