...
void parse_input_args(char **argv, int argc, FFConfig &ffConfig,MoeConfig &mconfig, TransformerConfig &tconfig) {
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "--batchsize")) {
      ffConfig.batchSize = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--microbatchsize")) {
      ffConfig.microbatchsize = atoi(argv[++i]);
      continue;
    }

    if (!strcmp(argv[i], "--dataset")) {
      mconfig.dataset_path = std::string(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--topk")) {
      mconfig.num_select = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--expnum")) {
      mconfig.num_exp = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--num-layers")) {
      tconfig.num_layers = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--embedding-size")) {
      tconfig.embedding_size = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--hidden-size")) {
      tconfig.hidden_size = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--expert-hidden-size")) {
      tconfig.expert_hidden_size = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--num-heads")) {
      tconfig.num_heads = atoi(argv[++i]);
      continue;
    }
    if (!strcmp(argv[i], "--sequence-length")) {
      tconfig.sequence_length = atoi(argv[++i]);
      continue;
    }
  }
}
...
Tensor create_moe_decoder(FFModel *model,
                          FFConfig const *ffConfig,
                          MoeConfig const *moeConfig,
                          TransformerConfig const *tfConfig,
                          Tensor const &input, int batch_size) {
...
void FlexFlow::top_level_task(Task const *task,
  ...
  MoeConfig moeConfig;
  {
    ...
    parse_input_args(argv, argc, ffConfig, moeConfig, tfConfig);
    // assert(ffConfig.num_microbatch % ffConfig.train_pp == 0);
    ffConfig.layer_per_stage = tfConfig.num_layers / ffConfig.train_pp;
  ...
  Tensor input;
  {
    int const dims[] = {ffConfig.batchSize * tfConfig.sequence_length, tfConfig.hidden_size};//64*512
  ...
  Tensor t = create_moe_decoder(&ff, &ffConfig,&moeConfig, &tfConfig, input, ffConfig.batchSize);
  ...
  ff.compile_taskgraph();

  // // Data Loader
  // log_app.print("In Data Loader");
  ...
  // log_app.print("Before init operators");
  // ff.init_operators();
  // log_app.print("After init operators");

  // // Start timer
  // log_app.print("Start timer");
  // {
...
DataLoader::DataLoader(FFModel &ff,
  ...
  // Create full input
  {
    ...
    // assert(input->num_dims == 4); // three dimensions + the replica dimension
    batch_input = input;
...
void DataLoader::load_entire_dataset(Task const *task,
  ...
  for (size_t i = 0; i < rect_label.volume(); i++) {
    label_ptr[i] = std::rand() % regions.size();
  }
}
