...
Tensor create_attention_encoder(FFModel *model,
  ...
  for (int i = 0; i < tfConfig->num_layers; i++) {
    x = model->layer_norm(model->multihead_attention(
        x, x, x, hidden_dim, num_heads, kdim, vdim), axes, true, 1e-05);
    x = model->dense(model->layer_norm(model->dense(x, hidden_dim, AC_MODE_RELU, false /*bias*/), axes, true, 1e-05),
                     hidden_dim,
                     AC_MODE_NONE,
                     false /*bias*/);
  }
...
Tensor create_attention_decoder(FFModel *model,
                                TransformerConfig const *tfConfig,
                                Tensor const &input,
                                int hidden_dim,
                                int num_heads,
  ...
  for (int i = 0; i < tfConfig->num_layers; i++) {
    x = model->layer_norm(
        ...
        true,
        1e-05);
  x = model->layer_norm(model->add(model->dense(x, tfConfig->hidden_size, AC_MODE_RELU, false /*bias*/),
                                   x),
                        axes,
                        true,
                        1e-05);
  }
  x = model->dense(x, tfConfig->hidden_size, AC_MODE_RELU, false /*bias*/);
  return x;
}
...
void parse_input_args(char **argv, int argc, FFConfig &ffConfig, TransformerConfig &config) {
...
void FlexFlow::top_level_task(Task const *task,
  ...
  TransformerConfig tfConfig;
  {
    ...
    parse_input_args(argv, argc,ffConfig, tfConfig);
  ...
  // Tensor t = create_attention_encoder(&ff,
  //                                     &tfConfig,
  //                                     input,
  //                                     tfConfig.hidden_size,
  //                                     tfConfig.num_heads,
  ...
  Tensor t = create_attention_decoder(&ff,
                                      &tfConfig,
                                      input,
                                      tfConfig.hidden_size,
                                      tfConfig.num_heads,
