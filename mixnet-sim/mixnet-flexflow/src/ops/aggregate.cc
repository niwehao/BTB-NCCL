...
bool Aggregate::measure_operator_cost(Simulator *sim,
  ...
  printf("Aggregate: numInputs-%d",numInputs);
  ParallelTensorBase sub_inputs[MAX_NUM_INPUTS], sub_pred, sub_assign,
      sub_output;
  MachineView mv_dp=mv;
  int ori_batch_size=inputs[4]->dims[1].size;
  int scale=ori_batch_size / (32*mv.dim[0]);
  mv_dp.dim[0]*=scale;
  //mv_dp.dim[0]=2;
  for (int i = 0; i < n; ++i) {
    if (!inputs[i + 4]->get_sub_tensor(mv_dp, sub_inputs[i])) {
      return false;
    }
  }
  if (!inputs[0]->get_sub_tensor(mv_dp, sub_pred)) {
    return false;
  }
  if (!inputs[1]->get_sub_tensor(mv_dp, sub_assign)) {
    return false;
  }

  if (!outputs[0]->get_sub_tensor(mv_dp, sub_output)) {
    return false;
  }
  fprintf(stderr,"ori_batch_size: %d,scale:%d, now dim:%d,sub_inputs[4]: dim[0]:%d,dim[1]:%d,dim[2]:%d\n",ori_batch_size,scale,mv_dp.dim[0],sub_inputs[4].dims[0].size,sub_inputs[4].dims[1].size,sub_inputs[4].dims[2].size);
  ...
  Domain pre_domain = sub_pred.get_domain();
  assert(batch_size==pre_domain.hi()[1] - pre_domain.lo()[1] + 1);

  int rows = exp_domain.hi()[1] - exp_domain.lo()[1] + 1; //8
  int out_dim = exp_domain.hi()[0] - exp_domain.lo()[0] + 1;//1024
  assert(out_dim == output_domain.hi()[0]-output_domain.lo()[0]+1);
  assert(batch_size==output_domain.hi()[1] - output_domain.lo()[1] + 1);

  //fortest
  for(int i=0;i<n;i++){
    Domain exp_domain_tmp = sub_inputs[i].get_domain();
    assert(rows == exp_domain_tmp.hi()[1] - exp_domain_tmp.lo()[1] + 1);
    assert(out_dim == exp_domain_tmp.hi()[0] - exp_domain_tmp.lo()[0] + 1);
  }

  forward = [&] {
    forward_kernel_wrapper(m,
                           input_ptrs, //float **exp_preds
                           assign_ptr, //int const *acc_gate_assign_ptr,
                           pred_ptr, //float const *acc_gate_pred_ptr,
                           output_ptr, //float *acc_output_ptr,
                           n,
                           k,
                           rows,
                           batch_size,
  ...
  int *assign_true_ptr = (int *)sim->allocate(sub_assign.get_volume(), DT_INT32);
  float *pred_grad_ptr = (float *)sim->allocate(sub_pred.get_volume(), DT_FLOAT);
  cost_metrics.inputs_memory += cost_metrics.total_mem_diff_from(sim->offset);
  ...
  cost_metrics.inputs_memory*=scale;
  cost_metrics.outputs_memory*=scale;
  cost_metrics.forward_time *=scale;
  cost_metrics.backward_time*=scale;
