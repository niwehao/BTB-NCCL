...
bool Softmax::measure_operator_cost(Simulator *sim,
  ...
  fprintf(stderr,"softmax output-1: %zu, output-2: %zu, output-3: %zu\n",sub_output.dims[0].size,sub_output.dims[1].size,sub_output.dims[2].size);
  ...
  fprintf(stderr,"first softmax su boutput_volume: %zu, total output memory: %zu \n",sub_output.get_volume(),cost_metrics.outputs_memory);
  ...
  if (sim->computationMode == COMP_MODE_TRAINING) {
    ...
    fprintf(stderr,"second softmax su boutput_volume: %zu, total output memory: %zu \n",sub_output.get_volume(),cost_metrics.outputs_memory);
    backward = [&] {
      backward_kernel_wrapper(
