...
LayerNorm::LayerNorm(FFModel &model,
  ...
  fprintf(stderr,
              "layer norm effective_batch_size:%d, effective_num_elements:%d \n",effective_batch_size,M);
...
bool LayerNorm::measure_operator_cost(Simulator *sim,
  ...
  m->effective_num_elements = mv.dim[0]*mv.dim[2];
  m->effective_batch_size = sub_input.get_volume()/m->effective_batch_size;

  // m->effective_batch_size = mv.dim[0]*mv.dim[2];
  // m->effective_num_elements = sub_input.get_volume()/m->effective_batch_size;

  fprintf(stderr,
              "layer norm effective_batch_size:%d \n",m->effective_batch_size);
  ...
  cost_metrics.forward_time /=10;
  cost_metrics.backward_time/=10;
  return true;
}
