...
__global__ void gb_rand_generate_int32(int32_t* ptr, size_t size, int32_t p)
{
...
void Group_by::gb_rand_generate_int32_wrapper(int32_t *ptr, size_t size, int32_t n) {
  cudaStream_t stream;
  checkCUDA(get_legion_stream(&stream));
  // Randomly initialize the intput tensor to avoid out of index range issues
  gb_rand_generate_int32<<<GET_BLOCKS(size), CUDA_NUM_THREADS, 0, stream>>>(
      ptr, size, n);
}
...
__global__ void
  ...
  if (threadIdx.x == 0) {
    //printf("Groupby: enter gb_forward_kernel\n");
    ...
    for (int i = 0; i < k * batch_size; i++) {
      ...
      int expert = exp_assign[i];//yijun we may need to check the range of expert
       //printf("Groupby: enter gb_forward_kernel\n");
      //printf("%d ",expert);
...
__global__ void
  ...
  assert(n <= MAX_N);
  //std::cout << "enter groupby cu" <<std::endl;
