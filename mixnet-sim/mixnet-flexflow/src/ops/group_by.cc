...
void Group_by::forward(FFModel const &ff) {
  printf("Groupby: enter Group_by::forward\n");
  ArgumentMap argmap;
  Context ctx = ff.config.lg_ctx;
  Runtime *runtime = ff.config.lg_hlr;
...
void Group_by::forward_task(Task const *task,
                            ...
                            Context ctx,
                            Runtime *runtime) {
  printf("Groupby: enter Group_by::forward_task\n");
  // Get n, alpha
...
bool Group_by::measure_operator_cost(Simulator *sim,
  ...
  int ori_batch_size=inputs[0]->dims[1].size;
  int scale=ori_batch_size / (64*mv.dim[0]);
  mv_dp.dim[0]*=scale;
  fprintf(stderr,"ori_batch_size: %d,scale:%d, now dim:%d \n",ori_batch_size,scale,mv_dp.dim[0]);


  if (!inputs[0]->get_sub_tensor(mv_dp, sub_input)) {
    return false;
  }
  ...
  for (int i = 0; i < numOutputs; ++i) {
    if (!outputs[i]->get_sub_tensor(mv_dp, sub_outputs[i])) {
      return false;
    }
  }
  ...
  for (int i = 0; i < numOutputs; i++) {
    ...
    fprintf(stderr,"%d-thsoftmax output-1: %zu, output-2: %zu, output-3: %zu\n",i,sub_outputs[i].dims[0].size,sub_outputs[i].dims[1].size,sub_outputs[i].dims[2].size);
  }
  cost_metrics.outputs_memory += cost_metrics.total_mem_diff_from(sim->offset);
  fprintf(stderr,"groupby suboutput_volume: %zu, total output memory: %zu \n",sub_outputs[0].get_volume(),cost_metrics.outputs_memory);
  ...
  printf("subassign: k: %d,dim[1]:%d, dim[2]:%d \n",k,sub_assign.dims[1].size,sub_assign.dims[2].size);
  printf("subinput: batch: %d,data_dim:%d\n",batch_size,data_dim);

  gb_rand_generate_int32_wrapper(assign_ptr,sub_assign.get_volume(),n);
  ...
  int *assign_grad_ptr=(int *)sim->allocate(sub_assign.get_volume(), DT_INT32);
  cost_metrics.inputs_memory += cost_metrics.total_mem_diff_from(sim->offset);
  float *output_grad_ptrs[MAX_NUM_OUTPUTS];
  for (int i = 0; i < numOutputs; i++) {
    output_grad_ptrs[i] =
        (float *)sim->allocate(sub_outputs[i].get_volume(), DT_FLOAT);
  }
  cost_metrics.outputs_memory += cost_metrics.total_mem_diff_from(sim->offset);
  gb_rand_generate_int32_wrapper(assign_grad_ptr,sub_assign.get_volume(),n);
  backward = [&] {
