...
// // (wxc): for copy op, used in PP model generation
// Op::Op(OperatorType _otype,
//        DataType _dtype,
//        LayerID _layer_guid,
//        size_t _op_guid,
//        char *_name,
//        Legion::IndexSpace *_parallel_is,
//        ParallelTensor *_outputs,
//        ParallelTensor *_inputs,
//        ParallelParameter *_weights,
//        bool *_trainableInputs,
//        OpMeta **_meta,
//        int _numInputs, 
//        int _numWeights, 
//        int _numOutputs,
//        bool _profiling,
//        std::vector<ParallelDimMappingRecord>  *_parallel_dims_mapping)
//        : op_type(_otype), data_type(_dtype), layer_guid(_layer_guid), op_guid(_op_guid), numInputs(_numInputs), numWeights(_numWeights), numOutputs(_numOutputs), profiling(_profiling), parallel_dims_mapping(_parallel_dims_mapping) {
//         std::strcpy(name, _name);
//         // this->parallel_is = _parallel_is;
//         std::copy(_outputs, _outputs + MAX_NUM_OUTPUTS, this->outputs);
//         std::copy(_inputs, _inputs + MAX_NUM_INPUTS, this->inputs);
//         std::copy(_weights, _weights + MAX_NUM_WEIGHTS, this->weights);
//         std::copy(_trainableInputs, _trainableInputs + MAX_NUM_INPUTS, this->trainableInputs);
//         std::copy(_meta, _meta + MAX_NUM_WORKERS, this->meta);
//        }
  ...
  switch (layer->op_type) {
    case OP_INPUT: {
      ...
      if (config.only_data_parallel) {
        ...
        // Repartition *part = new Repartition(
        //     *this, pt, num_dims - 1, config.train_dp);
        Repartition *part = new Repartition(
            *this, pt, num_dims - 1, 2);
...
// void FFModel::generate_taskgraph(PCG::Graph &best_graph,
...
//     FFModel* model = this;
//     TaskLauncher launcher(CUSTOM_TASKGRAPH_TASK_ID,
...
void FFModel::compile(LossType loss_type,
  ...
  // Launch the graph optimize task
  {
    ...
    // Generate taskgraph according to best_graph and optimal_views
    //generate_taskgraph(*best_graph, optimal_views, comp_mode);
      ...
      for (int i = 0; i < layer->numWeights; i++) {
        ...
        for (auto const &op : operators) {
          if (op->layer_guid == layer->layer_guid) {
            ...
            //! TOCHECK
            // if (op->op_type == OP_MULTIHEAD_ATTENTION) {
            //   layer->numWeights = 0;
            // }
...
struct DefaultConfig {
  ...
  const static size_t workSpaceSize = (size_t)8 * 1024 * 1024 * 1024; // 2GB
...
void FFConfig::parse_args(char **argv, int argc) {
  for (int i = 1; i < argc; i++) {
    ...
    if ((!strcmp(argv[i], "--train_dp"))) {
      train_dp = atoi(argv[++i]);
      continue;
    }
    if ((!strcmp(argv[i], "--train_tp"))) {
      train_tp = atoi(argv[++i]);
      continue;
    }
    if ((!strcmp(argv[i], "--train_pp"))) {
      train_pp = atoi(argv[++i]);
      continue;
    }
    ...
    // if ((!strcmp(argv[i], "--layer_per_stage"))) {
    //   layer_per_stage = atoi(argv[++i]);
    //   continue;
    // }
  }
}
...
void register_flexflow_internal_tasks(Runtime *runtime,
  ...
  // Taskgraph generate
  {
    TaskVariantRegistrar registrar(CUSTOM_TASKGRAPH_TASK_ID, "Taskgraph Generate");
    registrar.add_constraint(ProcessorConstraint(Processor::TOC_PROC));
    registrar.set_leaf();
    if (pre_register) {
      Runtime::preregister_task_variant<TaskgraphSimulator::simulation_task>(
          registrar, "Taskgraph Generate Task");
    } else {
      if (enable_control_replication) {
        registrar.global_registration = false;
      }
      runtime->register_task_variant<TaskgraphSimulator::simulation_task>(
          registrar);
    }
  }
  // Taskgraph generate
  {
    TaskVariantRegistrar registrar(CUSTOM_TASKGRAPH_TASK_ID_1, "Taskgraph Generate 1");
    registrar.add_constraint(ProcessorConstraint(Processor::TOC_PROC));
    registrar.set_leaf();
    if (pre_register) {
      Runtime::preregister_task_variant<Simulator::simulation_task>(
          registrar, "Taskgraph Generate Task 1");
    } else {
      if (enable_control_replication) {
        registrar.global_registration = false;
      }
      runtime->register_task_variant<Simulator::simulation_task>(
          registrar);
    }
  }
  // Taskgraph generate
  {
    TaskVariantRegistrar registrar(CUSTOM_TASKGRAPH_TASK_ID_2, "Taskgraph Generate 2");
    registrar.add_constraint(ProcessorConstraint(Processor::TOC_PROC));
    registrar.set_leaf();
    if (pre_register) {
      Runtime::preregister_task_variant<LogicalTaskgraphBasedSimulator::simulation_task>(
          registrar, "Taskgraph Generate Task 2");
    } else {
      if (enable_control_replication) {
        registrar.global_registration = false;
      }
      runtime->register_task_variant<LogicalTaskgraphBasedSimulator::simulation_task>(
          registrar);
    }
  }
  // Graph optimize
