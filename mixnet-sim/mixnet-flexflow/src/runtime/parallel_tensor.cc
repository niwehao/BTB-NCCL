...
bool ParallelTensorBase::get_sub_input_tensor(MachineView const &mv,
  ...
  for (int i = 0; i < sub_tensor.num_dims; i++) {
    ...
    {
    case 0:
      sub_tensor.dims[0].size /= mv.dim[1];//tp
      break;
    case 1:
      sub_tensor.dims[1].size /= (mv.dim[0]*mv.dim[2]);//dp*pp*ep
      break;
    default:
      break;
    }
...
bool ParallelTensorBase::get_sub_tensor(MachineView const &mv,
                                        ParallelTensorBase &sub_tensor) const {
  sub_tensor.num_dims = this->num_dims;
  //fprintf(stderr,"sub_tensor.num_dims: %d", sub_tensor.num_dims);
  for (int i = 0; i < sub_tensor.num_dims; i++) {
    sub_tensor.dims[i] = this->dims[i];
    if (this->dims[i].parallel_idx != -1) {
      int idx = this->dims[i].parallel_idx;
      //fprintf(stderr,"parallel_idx:%d",idx);
      assert(idx >= 0);
      //assert(this->dims[i].degree == mv.dim[idx]);//syj, we may not use it here...
      sub_tensor.dims[i].size /= (mv.dim[idx]*mv.dim[2]);
