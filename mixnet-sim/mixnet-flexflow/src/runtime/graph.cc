...
//syj: create computation device and comm device
//leave network model to pkt level simulator,network_bandwidth is useless here
void Graph::create_resource_pool (int num_nodes,
...
std::unique_ptr<Graph> Graph::generate_pp_graph(Graph const &g, 
                               int train_pp, 
                               int layer_per_stage,
                               std::unordered_map<Node, MachineView> const &strategy,
                               std::unordered_map<Node, MachineView> &new_strategy,
                               std::vector<std::unordered_map<FlexFlow::Op const *, Node>> &op_to_node_map_mbatch, 
                               std::vector<std::unordered_map<int, Node>> &startNode_stage_list,
                               std::vector<std::unordered_map<int, Node>> &endNode_stage_list) const {
  auto pp_NodeEdge_logger = spdlog::basic_logger_mt("logger_generate_pp_graph_ne", "./logs/pp_graph/node_edge.txt",true);//overlap
  pp_NodeEdge_logger->set_level(spdlog::level::info);
  pp_NodeEdge_logger->info("init pp node&edge logger");
  pp_NodeEdge_logger->flush();


  auto pp_stage_logger = spdlog::basic_logger_mt("logger_generate_pp_graph_stage", "./logs/pp_graph/stage.txt",true);//overlap
  pp_stage_logger->set_level(spdlog::level::info);
  pp_stage_logger->info("init pp stage logger");
  pp_stage_logger->flush();

  fprintf(stderr,
              "start generate pp graph\n");
  
  //The new graph has original g.model, but train_pp * g.nodes, with unique guid
  assert(g.model!=nullptr);
  Graph *new_graph = new Graph(g.model);
  size_t guid = 0;
  fprintf(stderr,
              "cp new graph\n");
  ...
  for (int i = 0; i < train_pp; i++) {
    ...
    // create new op and node
    pp_NodeEdge_logger->info("microbatch-{}",i);
    //pp_edge_logger->info("microbatch-{}",i);
    int layernorm_count = 0;
    for (FlexFlow::Op const *dstOp : g.model->operators) {
      Node dstNode;
      dstNode.ptr = dstOp;
      dstNode.micro_batch_id=i;
      dstNode.layer_id=layernorm_count/2;
      pp_NodeEdge_logger->info("node-{}:{}, micro_batch_id:{}, layerid:{}",guid, get_operator_type_name(dstOp->op_type),dstNode.micro_batch_id,dstNode.layer_id);
      if(dstOp->op_type == OP_LAYERNORM && (layernorm_count/2)<=g.layer_num-1){
        layernorm_count++;
      }
      dstNode.guid = guid++;
      op_to_node_map[dstOp] = dstNode;
      //last dense belong to last layer 
      new_graph->add_node(dstNode);
    }
    pp_NodeEdge_logger->flush();
    // create edges
    for (FlexFlow::Op const * dstOp : g.model->operators) {
      std::vector<FlexFlow::Op const *> srcOpList;
      for (int j = 0; j < dstOp->numInputs; j++) {
        ...
        if (std::find(srcOpList.begin(), srcOpList.end(), srcOp) != srcOpList.end()) {
          continue;
        }
        new_graph->add_edge(srcNode, op_to_node_map[dstOp], dstOp->inputs[j]->owner_idx, j);
        srcOpList.push_back(srcOp);
        pp_NodeEdge_logger->info("edge FROM:{}_{}-TO-{}{}",get_operator_type_name(srcNode.ptr->op_type),dstOp->inputs[j]->owner_idx,get_operator_type_name(op_to_node_map[dstOp].ptr->op_type),j);
      }
      pp_NodeEdge_logger->flush();
      srcOpList.clear();
    }
    op_to_node_map_mbatch.push_back(op_to_node_map);
  }

  // identify stage start_op and end_op
  int NUM_LAYERNORM_PER_LAYER = 2;
  int NUM_LAYERNORM_PER_STAGE = NUM_LAYERNORM_PER_LAYER * layer_per_stage;
  for (int i = 0; i < train_pp; i++) {
    pp_stage_logger->info("microbatch-{}",i);
    std::unordered_map<FlexFlow::Op const *, Node> op_to_node_map = op_to_node_map_mbatch[i];
    std::unordered_map<int, Node> startNode_stage;
    std::unordered_map<int, Node> endNode_stage;
    Node InputNode;

    for (FlexFlow::Op const * dstOp : g.model->operators) {
      if (dstOp->op_type == OP_INPUT) {
        InputNode = op_to_node_map[dstOp];
        break;
      }
    }
    startNode_stage[0] = InputNode;
    pp_stage_logger->info("startnode: stage_{},node_{}_{}",0,InputNode.guid,get_operator_type_name(InputNode.ptr->op_type));

    int num_layernorm = 0;
    for (int j = 0; j < g.model->operators.size(); j++) {
      auto *dstOp = g.model->operators[j];
      if (dstOp->op_type == OP_LAYERNORM) {
        num_layernorm++;
        if (num_layernorm % NUM_LAYERNORM_PER_STAGE == 0) {
          if (num_layernorm / NUM_LAYERNORM_PER_STAGE == train_pp) {
            // last stage, store the next op, which is Dense
            auto *nextOp = g.model->operators[j + 1];
            assert(nextOp->op_type == OP_LINEAR);
            endNode_stage[num_layernorm / NUM_LAYERNORM_PER_STAGE - 1] = op_to_node_map[nextOp];
            pp_stage_logger->info("endnode: stage_{},node_{}_{}",num_layernorm / NUM_LAYERNORM_PER_STAGE - 1,op_to_node_map[nextOp].guid,get_operator_type_name(op_to_node_map[nextOp].ptr->op_type));
          }
          else {
            endNode_stage[num_layernorm / NUM_LAYERNORM_PER_STAGE - 1] = op_to_node_map[dstOp];
            pp_stage_logger->info("endnode: stage_{},node_{}_{}",num_layernorm / NUM_LAYERNORM_PER_STAGE - 1,op_to_node_map[dstOp].guid,get_operator_type_name(op_to_node_map[dstOp].ptr->op_type));
            auto *nextOp = g.model->operators[j + 1];
            startNode_stage[num_layernorm / NUM_LAYERNORM_PER_STAGE] = op_to_node_map[nextOp];
            pp_stage_logger->info("startnode: stage_{},node_{}_{}",num_layernorm / NUM_LAYERNORM_PER_STAGE,op_to_node_map[nextOp].guid,get_operator_type_name(op_to_node_map[nextOp].ptr->op_type));
          }
        }
      }
    }

    assert(startNode_stage.size() == train_pp);
    assert(endNode_stage.size() == train_pp);

    startNode_stage_list.push_back(startNode_stage);
    endNode_stage_list.push_back(endNode_stage);

  }
  pp_stage_logger->flush();
  pp_NodeEdge_logger->flush();

  std::unique_ptr<Graph> new_g = std::unique_ptr<Graph>(new_graph);
...
std::pair<std::unique_ptr<Graph>, std::unordered_map<Node, MachineView>>
  ...
  if (model->config.only_data_parallel) {
    auto expert_logger = spdlog::basic_logger_mt("machineview_expert_logger", "./logs/gpu_allocation/expert_logger.txt",true);//overlap
    expert_logger->set_level(spdlog::level::info);
    expert_logger->info("init expert info logger");
    expert_logger->flush();

    auto mv_logger = spdlog::basic_logger_mt("machineview_mv_logger", "./logs/gpu_allocation/machineview_logger.txt",true);//overlap
    mv_logger->set_level(spdlog::level::info);
    mv_logger->info("init machineview logger");
    mv_logger->flush();

    auto op_logger = spdlog::basic_logger_mt("machineview_op_logger", "./logs/gpu_allocation/op_logger.txt",true);//overlap
    op_logger->set_level(spdlog::level::info);
    op_logger->info("init machineview logger");
    op_logger->flush();


    auto opsize_logger = spdlog::basic_logger_mt("op size logger", "./logs/gpu_allocation/opsize_logger.txt",true);//overlap
    opsize_logger->set_level(spdlog::level::info);
    opsize_logger->info("init opsize logger");
    opsize_logger->flush();
    ...
    std::unordered_map<FlexFlow::Op const *, Node> op_to_node_map;
    std::cout<<"only_data_parallel graph ";
    std::unordered_map<int, std::vector<Node>> layer2node; //[layer_id, vec(node)], last layer is dense layer
    std::unordered_map<int, std::unordered_set<Node>> expertdict;
    std::unordered_map<int, std::unordered_map<int,std::vector<Node>>> layerexpertNodes;//[layerid,expid,node]
    int LN_per_layer=2;
    int LN_count=0;
    int layer_num=0; //the last layer include extra dense
    int expert_number=0;
    for (FlexFlow::Op const *dstOp : model->operators) {
      Node dstNode;
      dstNode.ptr = dstOp;
      // std::cout<<dstNode.ptr->op_type<<" ";
      ...
      for (int j = 0; j < dstOp->numInputs; j++) {
        ...
        input_oss <<"input-"<< j<<" num dims: "<< dstOp->inputs[j]->num_dims<< " " <<dstOp->inputs[j]->dims[0].size << " "<<dstOp->inputs[j]->dims[1].size<< " "<< dstOp->inputs[j]->dims[2].size <<"\n";
      }
      std::ostringstream output_oss;
      for (int j = 0; j < dstOp->numOutputs; j++) {
        output_oss <<"output-"<< j<<" num dims: "<< dstOp->outputs[j]->num_dims<< " dims: " <<dstOp->outputs[j]->dims[0].size << " "<<dstOp->outputs[j]->dims[1].size<< " "<< dstOp->outputs[j]->dims[2].size <<"\n";
      }
      opsize_logger->info("opname_{},node guid_{}, op input size_{}, op output size_{}",get_operator_type_name(dstOp->op_type),dstNode.guid,input_oss.str(),output_oss.str());
      opsize_logger->flush();
    }

    for (FlexFlow::Op const *dstOp : model->operators) {
      Node dstNode=op_to_node_map[dstOp];
      for (int j = 0; j < dstOp->numInputs; j++) {
        FlexFlow::Op const *srcOp = dstOp->inputs[j]->owner_op;
        assert(op_to_node_map.find(srcOp) != op_to_node_map.end());
        Node srcNode = op_to_node_map[srcOp];
        //new logic here
        /*
        1. create layer2node: [layer, vec(node)]
        2. create expertdict: [layer, allexpertnode]
        3. create layerexpertnode: [layer,expertid,node]
        */
        if(dstOp->op_type==OP_AGGREGATE && j>3){ 
          expertdict[layer_num].insert(srcNode);//agg->softmax->dense_2->dense_1->groupby
          FlexFlow::Op const *dense_op2=srcOp->inputs[0]->owner_op;
          FlexFlow::Op const *dense_op1=dense_op2->inputs[0]->owner_op;
          assert(op_to_node_map.find(dense_op2) != op_to_node_map.end());
          assert(op_to_node_map.find(dense_op1) != op_to_node_map.end());
          Node dense_Node2 =  op_to_node_map[dense_op2];
          Node dense_Node1 =  op_to_node_map[dense_op1];
          expertdict[layer_num].insert(dense_Node2);//dense_2
          expertdict[layer_num].insert(dense_Node1);//dense_1
          
          layerexpertNodes[layer_num][expert_number].push_back(srcNode);
          layerexpertNodes[layer_num][expert_number].push_back(dense_Node2);
          layerexpertNodes[layer_num][expert_number].push_back(dense_Node1);

          expert_logger->info("layernum_{}, expert_num_{},node-3_{}: Opname_{},node-2_{}: Opname_{}, node-1_{}: Opname_{}",layer_num,expert_number,srcNode.guid,get_operator_type_name(srcNode.ptr->op_type),dense_Node2.guid, get_operator_type_name(dense_Node2.ptr->op_type),dense_Node1.guid,get_operator_type_name(dense_Node1.ptr->op_type));
      ...
      op_logger->info("Node_{},OPname_{},CurLayer_{}",dstNode.guid,get_operator_type_name(dstNode.ptr->op_type),layer_num);
      if(dstOp->op_type == OP_LAYERNORM){
          LN_count++;
      }
      layer2node[layer_num].push_back(dstNode);
      if(LN_count == LN_per_layer && layer_num < model->config.num_layers-1){
          layer_num++;
          expert_number=0;
          LN_count=0;
      }
    ...
    // std::cout<<"\n";
    mv_logger->info("layernum_{},expertnum_{},totalOPnum_{}",layer_num,expert_number,model->operators.size());
    if(expert_number==0){
      graph->exp_num=1;
      assert(expertdict.empty());
    }
    else{
      graph->exp_num=expert_number;
    }
    
    graph->layer_num=layer_num;
    graph->dp_degree=model->config.train_dp;
    graph->tp_degree=model->config.train_tp;
    graph->pp_degree=model->config.train_pp;
    graph->global_batch_size=model->config.batchSize;
    graph->microbatchsize=model->config.microbatchsize;
    curr_best_graph = std::unique_ptr<Graph>(graph);

    if (model->config.train_tp == 0) { //dp only
      MachineView data_parallel_view;
      data_parallel_view.device_type = MachineView::GPU;
      data_parallel_view.ndims = 1;
      data_parallel_view.dim[0] = model->config.train_dp;
      ...
      MachineView tensor_parallel_view;
      tensor_parallel_view.device_type = MachineView::GPU;
      tensor_parallel_view.ndims = 2;//first dp, second tp, third pp (hidden)
      tensor_parallel_view.dim[0] = model->config.train_dp;//dp,
      tensor_parallel_view.dim[1] = model->config.train_tp;//tp
      tensor_parallel_view.dim[2] =  model->config.train_pp;//pp
      tensor_parallel_view.stride[0] = 1; // use stride to assign gpu device, seems useless here
      tensor_parallel_view.start_device_id = 0; // start is zero, seems useless here

      int cur_gpu_idx=0;
      int gpu_start_idx=0;
      int gpu_start_idx2=0;
      int layer_per_stage = (layer_num+1) / model->config.train_pp;
      
      for(int layer_id=0;layer_id<=layer_num;layer_id++){
        if(layer_id % layer_per_stage==0) {//0,i*pp
          gpu_start_idx=cur_gpu_idx;
        }
        cur_gpu_idx = gpu_start_idx;
        for(int i=0;i<layer2node[layer_id].size();i++){
          Node curr_node=layer2node[layer_id][i];
          MachineView mv=tensor_parallel_view;
          if(expertdict[layer_id].find(curr_node)==expertdict[layer_id].end()){
            if (expert_number==0)
            {
              mv.dim[0]=model->config.train_dp;
            }else{
             mv.dim[0]= model->config.train_dp * expert_number; 
            }
            
            mv.start_device_id=gpu_start_idx2;
          }
          else {
            // no need to change mv
          }
          // if(curr_node.ptr->op_type==OP_GROUP_BY || curr_node.ptr->op_type==OP_TOPK||curr_node.ptr->op_type==OP_AGGREGATE || curr_node.ptr->op_type==OP_INPUT ||curr_node.ptr->op_type== OP_REPARTITION) {
          //   mv.dim[1]=1; // this op dont have tp, so set tp=1
          // }
          curr_optimal_views[curr_node] = mv;
        }
        int gpu_inc=0;
        for(int i=0; i < model->config.train_dp;i++){
          for(int j=0; j < layer2node[layer_id].size();j++){
            Node curr_node=layer2node[layer_id][j];
            if(expertdict[layer_id].find(curr_node)==expertdict[layer_id].end()){
              MachineView mv=curr_optimal_views[curr_node];
              gpu_inc=0;
              if(expert_number>0){
                for(int exp_idx=0; exp_idx < expert_number; exp_idx++){
                  gpu_inc+=model->config.train_tp;
                }
              }
            }
          }
          ...
          gpu_inc=0;
          for(int exp_id=0;exp_id < expert_number; exp_id++){
            for(int k=0; k<layerexpertNodes[layer_id][exp_id].size(); k++){
              Node curr_node=layerexpertNodes[layer_id][exp_id][k];
              curr_optimal_views[curr_node].start_device_id=gpu_start_idx2+gpu_inc;
            }
            gpu_inc+=model->config.train_tp;
          }
          if(expert_number>0)
            cur_gpu_idx+=expert_number*model->config.train_tp;
          else{
            cur_gpu_idx+=model->config.train_tp;
          }
          gpu_inc=0;

        }

        if((layer_id+1) % layer_per_stage==0) {//0,i*pp
          gpu_start_idx2+=model->config.train_dp*expert_number*model->config.train_tp;
        }
      }

      assert(curr_optimal_views.size()==model->operators.size());
      int totalgpu_num;
      
      if(expert_number==0) {
        totalgpu_num = model->config.train_dp*model->config.train_tp*model->config.train_pp;
      }
      else {
        totalgpu_num = model->config.train_dp*model->config.train_tp*model->config.train_pp*expert_number;
      }
      graph->create_resource_pool((totalgpu_num)/8,8);
      fprintf(stderr,
              "id_to_gpu size:%zu\n", curr_best_graph->id_to_gpu.size());
      fprintf(stderr,
              "config dp: %d, config tp:%d, config pp: %d, config layer_per_stage: %d\n", model->config.train_dp, model->config.train_tp, model->config.train_pp, model->config.layer_per_stage);
      expert_logger->flush();
      mv_logger->flush();
      op_logger->flush();
    }
    ...
    assert(false && "We assume MoE with DP by default.");
  }
...
GraphOptimalViewSerialized
  ...
  std::cout<<"ready print_strategy_computation_graph\n";
  best_graph->print_strategy_computation_graph(optimal_views);
  best_graph->export_strategy_computation_graph(optimal_views,"./output.txt");
  std::cout<<"finish print_strategy_computation_graph\n";

  // std::unique_ptr<Graph> new_g = std::move(best_graph->generate_task_graph(optimal_views, model_config.train_pp, model_config.layer_per_stage));
  best_graph->generate_task_graph(optimal_views, model_config.train_pp, model_config.layer_per_stage);

  std::string fname = "./results/taskgraph.fbuf";
  best_graph->searlize_logical_taskgraph(best_graph->model, fname);

  // // Following lines are to serialize the optimized PCG.
  // // Only need new_g and optimal_views below.
  // Serializer sez;
  // // First serialize graph
  // sez.serialize(new_g->inEdges.size());
  // std::unordered_map<Node, int> todos;
  // std::vector<Node> opList;
  // for (auto const &it : new_g->inEdges) {
  //   auto const &inList = it.second;
  //   todos[it.first] = (int)inList.size();
  //   if (todos[it.first] == 0) {
  //     opList.push_back(it.first);
  ...
  //   auto const &outList = new_g->outEdges[cur_node];
  //   for (auto const &e : outList) {
  //     todos[e.dstOp]--;
  //     if (todos[e.dstOp] == 0) {
  ...
  //     }
  //   }
  //   auto const &inList = new_g->inEdges[cur_node];
  ...
  //       sez.serialize(attn->is_moe);
  //       sez.serialize(attn->batch_size);
  //       break;
  //     }
  //     case OP_SOFTMAX: {
  ...
  // }
  // assert(node_idx == new_g->inEdges.size());
  ...
  // // delete new_g;
  // return ret;
  return GraphOptimalViewSerialized{};  // TODO(wxc): null return
}
  ...
  for (size_t node_idx = 0; node_idx < num_nodes; node_idx++) {
    ...
    switch (op_type) {
      ...
      case OP_MULTIHEAD_ATTENTION: {
        ...
        float dropout;
        bool bias, add_bias_kv, add_zero_attn, is_moe;
        int batch_size;
        size_t id;
        dez.deserialize(id);
        LayerID layer_guid(id);
        ...
        dez.deserialize(is_moe);
        dez.deserialize(batch_size);
