...
#include "flexflow/taskgraph_generated.h"
...
void TaskManager::reset(size_t  dp,size_t tp,size_t pp,size_t ep) {
  global_task_id = 0;
  dp_degree=dp;
  tp_degree=tp;
  pp_degree=pp;
...
SimTask *TaskManager::new_forward_task(Op const *op, int idx, int micro_batch, int layer_id) {
  SimTask *task = new_task();
  task->type = SimTask::TASK_FORWARD;
  size_t hash = 17 * 31 + (size_t)(op);
  hash = hash * 31 + std::hash<int>()(idx);
  ...
  task->name = op->name;
  task->hash_value=hash;
  task->micro_batch_id=micro_batch;
...
SimTask *TaskManager::new_backward_task(Op const *op, int idx,int micro_batch, int layer_id) {
  SimTask *task = new_task();
  task->type = SimTask::TASK_BACKWARD;
  size_t hash = 17 * 31 + (size_t)(op);
  hash = hash * 31 + std::hash<int>()(idx);
...
CostMetrics Simulator::measure_operator_cost(Op const *op,
  ...
  // printf("enter Simulator::measure_operator_cost\n");
...
void Simulator::fortest_helloworld(){
  printf("hello world\n");
}
...
bool LogicalTaskgraphBasedSimulator::serialize_logical_taskgraph(FFModel const *model, std::string const &export_file_name) {
  // TODO()
  return false;
}

void LogicalTaskgraphBasedSimulator::get_taskgraph_flatbuf(FFModel const *model, flatbuffers::FlatBufferBuilder &builder) {
  // TODO()
}
...
void TaskgraphSimulator::get_taskgraph_flatbuf(FFModel const *model, flatbuffers::FlatBufferBuilder &builder) {
  // TODO()
}

// old version of allreduce task 
SimTask* TaskManager::new_allreduce_task(Op *op, const std::vector<int> &node_ids, size_t message_size) 
{
  SimTask* task = new_task();
  task->name="TASK_ALLREDUCE";
...
SimTask *TaskManager::new_p2p_task(int src_id, int dst_id, size_t message_size) {
  SimTask *task = new_task();
  task->name="TASK_P2P";
  task->type = SimTask::TASK_P2P;
  task->from_node_ids.push_back(src_id);
  task->to_node_ids.push_back(dst_id);
  task->xfer_size = message_size;
  return task;
}

SimTask *TaskManager::new_p2p_task(std::vector<int> const &src_ids,
                                   std::vector<int> const &dst_ids,
                                   size_t message_size) {
  ...
  for (size_t i = 0; i < src_ids.size(); i++) {
    task->from_node_ids.push_back(src_ids[i]);
    task->to_node_ids.push_back(dst_ids[i]);
  }
  task->xfer_size = message_size;
  return task;
}
...
SimTask *TaskManager::new_p2p_task(std::vector<int> const &src_ids,
  ...
  task->micro_batch_id=micro_batch;
  task->layer_id=layer_id;
  task->target_micro_batch_id=TO_micro_batch;
  task->target_layer_id=TO_layer_id;
  switch (type) {
    case 1: {
      task->info = "forward p2p";
      break;
    }
    case 2: {
      task->info = "backward p2p";
      break;
    }
  }
  return task;
...
SimTask *TaskManager::new_sub_allreduce_task(int node_id){
  SimTask *task = new_task();
  task->type = SimTask::TASK_SUB_ALLREDUCE;
  task->task_info = reinterpret_cast<char*>(node_id);
  return task;
}

SimTask *TaskManager::new_dp_allreduce_task(
                                         std::vector<int> const &node_ids,
                                         size_t message_size) {
  SimTask *task = new_task();
  task->type = SimTask::TASK_DP_ALLREDUCE;
  for (size_t i = 0; i < node_ids.size(); i++) {
    //task->next_tasks.push_back(new_sub_allreduce_task(node_ids[i]));
    task->to_node_ids.push_back(node_ids[i]);
  }
  task->xfer_size = message_size;
  return task;
}

SimTask *TaskManager::new_tp_allreduce_task(
                                std::vector<int> const &node_ids,
                                size_t message_size) {
  SimTask *task = new_task();
  task->type = SimTask::TASK_TP_ALLREDUCE;
  for (size_t i = 0; i < node_ids.size(); i++) {
    // SimTask *subtask = new_sub_allreduce_task(node_ids[i]);
    // subtask->add_next_task(dst_task);
    // task->next_tasks.push_back(subtask);
    task->to_node_ids.push_back(node_ids[i]);
  }
  task->xfer_size = message_size;
  return task;
...
SimTask *TaskManager::new_allreduce_task(
                                         std::vector<int> const &node_ids,
                                         size_t message_size) {
  SimTask *task = new_task();
  task->name="TASK_ALLREDUCE";
...
SimTask *TaskManager::new_allreduce_task(
  ...
  switch (type) {
    case 1:{
      task->info="forward tp allreduce";
      break;
    }
    case 2:{
      task->info="backward tp allreduce";
      break;
    }
    case 3:{
      task->info="forward dp allreduce";
      break;
    }
    case 4:{
      task->info="backward dp allreduce";
      break;
    }
  }
  return task;
...
SimTask *TaskManager::new_alltoall_task( 
                                         std::vector<int> const &from_node_ids,
                                         std::vector<int> const &to_node_ids,
                                         size_t message_size) {
  SimTask *task = new_task();
  task->name="TASK_ALLTOALL";
  task->type = SimTask::TASK_ALLTOALL;
  for (size_t i = 0; i < from_node_ids.size(); i++) {
    task->from_node_ids.push_back(from_node_ids[i]);
  }
  for (size_t i = 0; i < to_node_ids.size(); i++) {
    task->to_node_ids.push_back(to_node_ids[i]);
  }
  task->xfer_size = message_size;
  return task;
}
...
SimTask *TaskManager::new_alltoall_task( 
  ...
  switch (type) {
    case 1:{
      task->info="GROUP_BY forward all2all";
      break;
    }
    case 2:{
      task->info="GROUP_BY backward all2all";
      break;
    }
    case 3:{
      task->info="AGGREGATE forward all2all";
      break;
    }
    case 4:{
      task->info="AGGREGATE backward all2all";
      break;
    }
  }
  return task;
